        ...
        with st.container(border=True):
            ...
            with a:
                st.markdown(
                    ...
                    f"<div><div style='font-weight:700;font-size:1.04rem;color:{theme.INK}'>"
                    f"{r['brand_name']} {theme.grade_chip(str(r['risk_grade']))} "
                    f"{theme.chip(cur.get('status', '미착수'), STATUS_KIND[cur.get('status', '미착수')])}"
                    f"</div>"
                    f"<div style='font-size:.79rem;color:{theme.TEXT_SUB}'>"
                ...
                if detail and detail != "nan":
                    st.markdown(
                        f"<div style='font-size:.86rem;color:{theme.TEXT};margin-top:9px;"
